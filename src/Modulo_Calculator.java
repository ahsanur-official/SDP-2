...
public class Modulo_Calculator extends JFrame {
    ...
    public Modulo_Calculator() {
        ...
        JLabel lblNewLabel = new JLabel("© 2025 MD. AHSANUR RAHAMAN. All rights reserved.");
        ...
        button_i.addActionListener(e -> displayArea.append("i"));
        button_i.setForeground(Color.WHITE);
        button_i.setFont(new Font("Tahoma", Font.BOLD, 20));
        button_i.setBackground(new Color(51, 51, 51));
        button_i.setBounds(12, 523, 79, 51);
        contentPane.add(button_i);

        JButton button_0 = new JButton("0");
        button_0.setBounds(103, 523, 79, 51);
     ...
     // --- Equals Button ---
        ...
        button_eq.setBounds(375, 335, 79, 239);
        button_eq.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent arg0) {
                String expr = displayArea.getText();
                if (expr.isEmpty()) return;
                try {
                    // *** Call the new, working parser ***
                    Complex result = ComplexParser.evaluate(expr);
    ...
    private class ImmediateCalculationListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            try {
                // *** Call the new, working parser ***
                Complex c = ComplexParser.evaluate(currentText);
                Complex res = null;
                switch (function) {
                    case "sqrt": res = c.sqrt(); break;
                    case "sqr": res = c.pow(new Complex(2, 0)); break; // Use complex pow
                    case "reciprocal": res = c.reciprocal(); break;
                    case "negate": res = c.negate(); break;
                    case "sin": res = c.sin(); break;
                    case "cos": res = c.cos(); break;
                    case "tan": res = c.tan(); break;
                    case "log": res = c.log10(); break;
                    case "ln": res = c.ln(); break;
                }
...
} // *** END OF Modulo_Calculator CLASS ***
